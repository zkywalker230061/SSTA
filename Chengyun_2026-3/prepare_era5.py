...
def save_turbulent_surface_stress():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Turbulent_Surface_Stress-(2004-2025).nc" in logs_datasets.read():
            return

    era5_turbulent_surface_stress = xr.open_dataset(
        "datasets/ERA5/ERA5_Mean_Turbulent_Surface_Stress.nc"
    )
...
def save_surface_heat_flux():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Surface_Heat_Flux-(2004-2025).nc" in logs_datasets.read():
            return

    era5_surface_heat_flux = xr.open_dataset(
        "datasets/ERA5/ERA5_Mean_Surface_Heat_Flux.nc"
    )
...
def save_surface_water_rate():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Surface_Water_Rate-(2004-2025).nc" in logs_datasets.read():
            return

    era5_surface_water_rate = xr.open_dataset(
        "datasets/ERA5/ERA5_Mean_Surface_Water_Rate.nc"
    )
