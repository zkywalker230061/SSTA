...
for month in water_rate_ds.TIME.values:
    ...
    if not added_baseline:  # just adds the baseline of a whole bunch of zero
        base = salinity_ds.sel(PRESSURE=2.5, TIME=month)['ARGO_SALINITY_ANOMALY'] - \
               salinity_ds.sel(PRESSURE=2.5, TIME=month)['ARGO_SALINITY_ANOMALY']
...
make_movie(all_anomalies_ds["IMPLICIT"], -1, 1)
...
make_movie(observed, -1, 1)

print(observed.max().item(), observed.min().item())
print(all_anomalies_ds["IMPLICIT"].max().item(), all_anomalies_ds["IMPLICIT"].min().item())

print(observed.mean().item(), all_anomalies_ds["IMPLICIT"].mean().item())

print(abs(observed).mean().item(), abs(all_anomalies_ds["IMPLICIT"]).mean().item())