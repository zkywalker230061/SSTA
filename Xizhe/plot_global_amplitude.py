...
T_SUB_DATA_PATH = "datasets/New_Entrainment/Tsub_Max_Gradient_Method_h.nc" ##-----------check if this is right
...
# --- 1. Prepare Global Forcing Tendencies (°C / month) ---
# We need the global conversion factor: delta_t / (rho_0 * c_0 * hbar)
# We take the time-mean of hbar to simplify the global spatial scaling
mean_hbar = hbar_da.mean(dim='MONTH')
global_conversion = delta_t / (rho_0 * c_0 * mean_hbar)

# Convert global fluxes to temperature tendencies 
qnet_tendency = flux_components_ds['SURFACE_FLUX_ANOMALY'] * global_conversion
ekman_tendency = flux_components_ds['EKMAN_FLUX_ANOMALY'] * global_conversion
entrain_tendency = flux_components_ds['ENTRAINMENT_FLUX_IMPLICIT_ANOMALY'] * global_conversion
# Assuming geo is already a tendency or needs similar scaling
geo_tendency = geostrophic_anomaly_da * global_conversion 

# --- 2. Calculate Temporal Standard Deviation ---
# This shows the typical magnitude/amplitude of the forcing at every location
std_qnet = qnet_tendency.std(dim='TIME')
std_ekman = ekman_tendency.std(dim='TIME')
std_entrain = entrain_tendency.std(dim='TIME')
std_geo = geo_tendency.std(dim='TIME')

# --- 3. Set up the 4-Panel Cartopy Plot ---
...
plot_data = [std_qnet, std_ekman, std_entrain, std_geo]
titles = ['a) Air-Sea Surface Forcing Amplitude', 'b) Ekman Forcing Amplitude', 
          'c) Entrainment Forcing Amplitude', 'd) Geostrophic Forcing Amplitude']

# Find a common maximum value so the colorbars are directly comparable
# (You may need to cap this if one forcing has extreme outliers)
vmax_val = np.nanpercentile([std_qnet.values, std_ekman.values, std_entrain.values, std_geo.values], 98)
...
for i, ax in enumerate(axes):
    ...
    # Plot the standard deviation using a sequential colormap (e.g., viridis, plasma, or YlOrRd)
    mesh = plot_data[i].plot(
        ax=ax, transform=ccrs.PlateCarree(),
        cmap='YlOrRd', vmin=0, vmax=vmax_val, add_colorbar=False, zorder=1
    )
...
cbar.set_label('Standard Deviation of Forcing (°C / month)', fontsize=12)

plt.subplots_adjust(bottom=0.12, hspace=0.1, wspace=0.1)
plt.savefig("Global_Forcing_Amplitudes.pdf", bbox_inches='tight', dpi=600)
plt.show()
