...
start_mode = 0
end_mode = 1
to_plot = all_schemes_ds["IMPLICIT"]
monthly_mean = get_monthly_mean(to_plot)
eof_modes, explained_variance, PCs = get_eof_with_nan_consideration(to_plot, modes=end_mode, mask=map_mask, tolerance=1e-15, monthly_mean_ds=monthly_mean, start_mode=start_mode)
...
print(eof_modes)
make_movie(eof_modes, -2, 2, "Anomaly from EOF Modes " + str(start_mode) + " to " + str(end_mode))
#make_movie(eof_modes_obs, -2, 2, "Anomaly from EOF Modes " + str(start_mode) + " to " + str(end_mode))
print("Variance explained:", explained_variance[start_mode:end_mode].sum().item())

# # plot PCs over time
k = 0
plt.grid()
plt.plot(to_plot.coords["TIME"].values, PCs[:, k], label="Simulated mode " + str(k))
plt.plot(to_plot.coords["TIME"].values, PCs_obs[:, k], label="Observed mode " + str(k))
plt.xlabel("Time (months since January 2004)")
plt.ylabel("PC")
plt.legend()
plt.show()
