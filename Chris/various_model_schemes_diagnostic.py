...
# make_movie(all_schemes_ds["IMPLICIT"], -4, 4, colorbar_label="Implicit Scheme", ENSO_ds=enso_indices_ds)
...
# make_movie(observed_anomaly, -10, 10, colorbar_label="Argo Anomaly", ENSO_ds=enso_indices_ds)
...
if MAKE_REGRESSION_MAPS:
    ...
    PCs_xr = xr.DataArray(PCs, dims=['TIME', 'MODE'], coords={'TIME': to_plot.TIME, 'MODE': np.arange(PCs.shape[1])})
    correlation_maps = correlation_map(PCs, field.values) * field.std(dim='TIME').values
    ...
    if CONSIDER_OBSERVATIONS:
        ...
        PCs_obs_xr = xr.DataArray(PCs_obs, dims=['TIME', 'MODE'], coords={'TIME': to_plot.TIME, 'MODE': np.arange(PCs_obs.shape[1])})
        correlation_maps_obs = correlation_map(PCs_obs, field_obs.values) * field_obs.std(dim='TIME').values
