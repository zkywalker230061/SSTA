...
def compare_mlds(mld_list, obs):
    ...
    autocorrelations = []
    for mld in mld_list:
        model_name = get_save_name(INCLUDE_SURFACE, INCLUDE_EKMAN_ANOM_ADVECTION, INCLUDE_ENTRAINMENT, INCLUDE_GEOSTROPHIC_ANOM_ADVECTION, USE_DOWNLOADED_SSH=False, gamma0=15.0, INCLUDE_GEOSTROPHIC_DISPLACEMENT=INCLUDE_GEOSTROPHIC_MEAN_ADVECTION, INCLUDE_EKMAN_MEAN_ADVECTION=INCLUDE_EKMAN_MEAN_ADVECTION, OTHER_MLD=False, MAX_GRAD_TSUB=True, ENTRAINMENT_VEL_ANOM_FORC=INCLUDE_ENTRAINMENT_VEL_ANOMALY_FORCING, LOG_ENTRAINMENT_VELOCITY=False, SPLIT_SURFACE=SPLIT_SURFACE, INCLUDE_RADIATIVE_SURFACE=INCLUDE_RADIATIVE_SURFACE, INCLUDE_TURBULENT_SURFACE=INCLUDE_TURBULENT_SURFACE, DATA_TO_2025=True, adjust_mld=mld)
    ...
    fig, ax = plt.subplots()
    plt.grid()
    plt.scatter(mld_list, mean_nrmse, marker='x')
    plt.xlabel("Mixed Layer Depth Adjustment (dbar)")
    plt.ylabel("Mean Model NRMSE with respect to Observations")
    fig.patch.set_alpha(0)
    ax.patch.set_alpha(0)
    grad, yint = np.polyfit(mld_list[2:-2], mean_nrmse[2:-2], 1)
    print("best fit parameters")
    print(grad)
    print(yint)
    mean_nrmse_fit = []
    mld_fit = np.linspace(-15, 15, 100)
    for mld in mld_fit:
        mean_nrmse_fit.append(grad * mld + yint)
    plt.plot(mld_fit, mean_nrmse_fit)
    plt.savefig("/Volumes/G-DRIVE ArmorATD/Extension/datasets/results_for_report/mld_sensitivity_nrmse.png", dpi=400, transparent=True, bbox_inches='tight')
    plt.show()
    ...
    for i, mld in enumerate(mld_list):
        ...
        label = "MLD deepened by " + str(mld) + " dbar"
        if mld == 0.0:
    ...
    fig.patch.set_alpha(0)
    ax.patch.set_alpha(0)
    plt.savefig("/Volumes/G-DRIVE ArmorATD/Extension/datasets/results_for_report/mld_sensitivity_autocorrelation.png", dpi=400, transparent=True, bbox_inches='tight')
    #plt.show()

mld_list = [-40, -20, -10, -5, 0.0, 5, 10, 20, 40]
# mld_list = [1.0, 0.0]
compare_mlds(mld_list, observed_anomaly)
