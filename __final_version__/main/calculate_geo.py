...
def save_alpha_beta():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/beta.nc" in logs_datasets.read():
            return

    t = load_and_prepare_dataset("datasets/Temperature-(2004-2025).nc")
    s = load_and_prepare_dataset("datasets/Salinity-(2004-2025).nc")
    sa = gsw.SA_from_SP(s.SALINITY, s.PRESSURE, s.LONGITUDE, s.LATITUDE)
    ct = gsw.CT_from_t(sa, t.TEMPERATURE, s.PRESSURE)

    sa.attrs['units'] = 'g/kg'
    sa.attrs['long_name'] = (
        'Absolute Salinity Jan 2004 - Dec 2025 (22.0 year)'
    )
    sa.name = 'SA'

    ct.attrs['units'] = '°C'
    ct.attrs['long_name'] = (
        'Conservative Temperature Jan 2004 - Dec 2025 (22.0 year)'
    )
    ct.name = 'CT'

    sa.to_netcdf("datasets/SA.nc")
    ct.to_netcdf("datasets/CT.nc")
    ...
    alpha.attrs['long_name'] = (
        'Thermal Expansion Coefficient Jan 2004 - Dec 2025 (22.0 year)'
    )
    alpha.name = 'alpha'

    beta.attrs['units'] = '1/PSU'
    beta.attrs['long_name'] = (
        'Haline Contraction Coefficient Jan 2004 - Dec 2025 (22.0 year)'
    )
    beta.name = 'beta'
...
def save_sea_surface_height():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Sea_Surface_Height-(2004-2025).nc" in logs_datasets.read():
            return
    ...
    ssh.attrs['long_name'] = (
        'Monthly Sea Surface Height Jan 2004 - Dec 2025 (22.0 year)'
    )
    ssh.name = 'ssh'

    save_file(
        ssh,
        "datasets/Sea_Surface_Height-(2004-2025).nc"
    )
...
def save_monthly_mean_sea_surface_height():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Sea_Surface_Height-Clim_Mean.nc" in logs_datasets.read():
            return
    ...
    save_file(
        ssh_monthly_mean,
        "datasets/Sea_Surface_Height-Clim_Mean.nc"
    )
...
def save_sea_surface_height_anomalies():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Sea_Surface_Height_Anomalies-(2004-2025).nc" in logs_datasets.read():
            return
    ...
    save_file(
        ssh_a,
        "datasets/Sea_Surface_Height_Anomalies-(2004-2025).nc"
    )
...
def save_geostrophic_anomaly_temperature():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Simulation-Geostrophic_Heat_Flux-(2004-2025).nc" in logs_datasets.read():
            return
    ...
    t_m_monthly_mean = xr.concat([t_m_monthly_mean] * 22, dim='MONTH').reset_coords(drop=True)
    t_m_monthly_mean = t_m_monthly_mean.rename({'MONTH': 'TIME'})
    t_m_monthly_mean['TIME'] = t_m_a.TIME
    ssh_monthly_mean = xr.concat([ssh_monthly_mean] * 22, dim='MONTH').reset_coords(drop=True)
    ...
    q_geostrophic_a.attrs['long_name'] = (
        'Monthly Q_Geostrophic Anomaly Jan 2004 - Dec 2025 (22.0 year)'
    )
...
def save_geostrophic_anomaly_salinity():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Simulation-Geostrophic_Water_Rate-(2004-2025).nc" in logs_datasets.read():
            return
