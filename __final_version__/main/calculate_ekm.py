...
def save_monthly_mean_turbulent_surface_stress():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Turbulent_Surface_Stress-Clim_Mean.nc" in logs_datasets.read():
            return

    tao_ds = load_and_prepare_dataset(
        "datasets/Turbulent_Surface_Stress-(2004-2025).nc"
    )
    ...
    save_file(
        tao_monthly_mean_ds,
        "datasets/Turbulent_Surface_Stress-Clim_Mean.nc"
    )
...
def save_turbulent_surface_stress_anomalies():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Turbulent_Surface_Stress_Anomalies-(2004-2025).nc" in logs_datasets.read():
            return
    ...
    save_file(
        tao_a_ds,
        "datasets/Turbulent_Surface_Stress_Anomalies-(2004-2025).nc"
    )
...
def save_ekman_anomaly_temperature():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Simulation-Ekman_Heat_Flux-(2004-2025).nc" in logs_datasets.read():
            return
    ...
    q_ekman_a.attrs['long_name'] = (
        'Monthly Q_Ekman Anomaly Jan 2004 - Dec 2025 (22.0 year)'
    )
...
def save_ekman_anomaly_salinity():
    ...
    with open("logs/datasets.txt", "r", encoding="utf-8") as logs_datasets:
        if "datasets/Simulation-Ekman_Water_Rate-(2004-2025).nc" in logs_datasets.read():
            return
    ...
    q_ekman_a = 1 / f * (
        tao_a_ds['ANOMALY_avg_iews'] * ds_m_monthly_mean_dy_da -
        tao_a_ds['ANOMALY_avg_inss'] * ds_m_monthly_mean_dx_da
    )
