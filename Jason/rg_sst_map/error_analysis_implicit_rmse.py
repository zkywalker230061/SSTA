...
def calculate_variance_ratio(obs, model, dim="TIME"):
    std_obs = obs.std(dim=dim)
    std_model = model.std(dim=dim)

    return std_model / std_obs
...
def decompose_mse(obs, model, dim='TIME'):
    ...
    # 2. Correlation (r)
    # Ensure you handle NaNs properly for valid grid points
...
results = decompose_mse(observed_temperature_anomaly, implicit_model_anomaly_ds)
...
print(bias_da)
print(implicit_model_anomaly_ds.mean(dim="TIME")**2)

fig, axes = plt.subplots(2, 2, figsize=(20, 10))
axes = axes.flatten()  

names = ["Bias Error", "Variance Error", "Phase Error", "Total MSE"]
maps_to_plot = [bias_da, var_da, phase_da, total_mse_da]

for i, item in enumerate(maps_to_plot):
    im = item.plot(ax=axes[i], cmap='viridis', add_colorbar=True, 
                   cbar_kwargs={'label': 'MSE Contribution'})
    
    axes[i].set_title(f'{names[i]}')
    
  
    mean_val = item.mean().item()
    print(f"{names[i]} Mean: {mean_val:.4f}")

plt.tight_layout()
fig.suptitle(f"MSE Decomposition (Gamma={gamma_0})", fontsize=20)
plt.tight_layout()
plt.show()
...
rmse_map = calculate_RMSE_normalised(observed_temperature_anomaly, implicit_model_anomaly_ds, dim='TIME')
...
scheme_name = "Implicit"
variance_map = calculate_variance_ratio(observed_temperature_anomaly, implicit_model_anomaly_ds)


variance_map.plot(ax=axes, cmap='nipy_spectral', cbar_kwargs={'label': 'RMSE (K)'}, vmin = 0, vmax = 2)
axes.set_xlabel("Longitude")
axes.set_ylabel("Lattitude")
axes.set_title(f'{scheme_name} Scheme - Variance Map')
max_var = variance_map.max().item()
print("Maximum Var Value", scheme_name, max_var)
max_var_location = variance_map.where(variance_map == variance_map.max(), drop=True).squeeze()
print("Maximum Var Location:", max_var_location)
...
test_plot = abs((ekman_anomaly_da.mean(dim="TIME")))
test_plot.plot(ax=axes, cmap='nipy_spectral', cbar_kwargs={'label': 'Temperature Mean Anomaly (K)'}, vmin = 0, vmax = 0.01)
axes.set_xlabel("Longitude")
axes.set_ylabel("Lattitude")
axes.set_title(f'{scheme_name} Scheme - Mean Anomaly Plot')
plt.tight_layout()
fig.text(
    0.99, 0.01,
    f"Gamma = {gamma_0}\n"
...
test_plot2 = entrainment_flux_implicit.mean(dim="TIME")
print(test_plot2)
